...
export default {
  ...
  asyncData({ params, error }) {
    //调用
    return hospApi.getHospitalList(null).then(response => {
      return {
  ...
  methods:{
    ...
    init() {
      //查询医院等级列表
      dictApi.getChildListByPid('10000').then(response => {
        //hostypeList清空
        this.hostypeList = []
        //向hostypeList添加全部值
      ...
      //查询地区数据
      dictApi.getChildListByPid('110100').then(response => {
        this.districtList = []
    ...
    //查询医院列表
    getList() {
      hospApi.getHospitalList(this.searchObj).then(response => {
